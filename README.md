...
[![Open in Codespaces](https://classroom.github.com/assets/launch-codespace-2972f46106e565e64193e422d61a12cf1da4916b45550586e14ef0a7c637dd04.svg)](https://classroom.github.com/open-in-codespaces?assignment_repo_id=21427985)
...
A **templated doubly linked list** used as the internal container for all linked-list-based structures (`LLS`, `LLQ`,
`LLDQ`).
...
**All LinkedList-based implementations must properly follow the Big Five** since the list allocates nodes dynamically (
`new Node()`).
...
Each inherits no internal data, relying on linked or array containers for definition. All functions mentioned above will
be overriden inside the derived classes, and **each interface should be defined as Abstract Base Classes.**
...
All implementors of `StackInterface`, `QueueInterface`, or `DequeInterface` **must fully define the Big Five** due to
dynamic memory.
...
Like all link-based structures, LLDQ has **O(1)** double-ended operations and **must follow the Big 5** to manage
internal state safely.
...
3. **Interfaces define contract, not storage.**
   None of the interfaces inherit `LinkedList<T>` — instead, concrete `LLS`, `LLQ`, and `LLDQ` **include** a list as a
   private attribute.
